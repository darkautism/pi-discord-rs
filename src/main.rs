use agent::{AgentEvent, AiAgent, NoOpAgent, ContentType};
use clap::{Parser, Subcommand, CommandFactory};
use rust_embed::RustEmbed;
use serenity::all::{
    CommandInteraction, Context, CreateEmbed, CreateMessage, EditMessage, EventHandler, GatewayIntents,
...
use composer::{BlockType, EmbedComposer, Block};
use config::Config;
...
#[derive(Subcommand)]
enum DaemonAction { Enable, Disable }
...
impl I18n {
    fn new(lang: &str) -> Self {
        ...
        let content = if let Some(file) = Asset::get(&path) {
            std::str::from_utf8(file.data.as_ref()).expect("UTF-8").to_string()
        } else {
            r#"{"processing": "...", "wait": "..."}"#.to_string()
        };
        I18n { texts: serde_json::from_str(&content).expect("JSON") }
    }
    fn get(&self, key: &str) -> String { self.texts.get(key).and_then(|v| v.as_str()).unwrap_or(key).to_string() }
    fn get_arg(&self, key: &str, arg: &str) -> String { self.get(key).replace("{}", arg) }
}
...
fn load_all_prompts() -> String {
    ...
    files.into_iter().map(|(_, c)| c).collect::<Vec<_>>().join("\n\n")
}

struct Handler { state: AppState }

#[derive(Clone, Debug, PartialEq)]
enum ExecStatus { Running, Success, Error(String) }
...
impl Handler {
    async fn start_agent_loop(
        ...
        let mut discord_msg = match channel_id.send_message(&http, CreateMessage::new().embed(CreateEmbed::new().title(&processing_msg).color(0xFFA500))).await {
            Ok(m) => m,
            Err(e) => { error!("Failed to send: {}", e); return; }
        };
        ...
        if let Some(msg) = initial_message {
            ...
            if is_brand_new {
                ...
                if !prompts.is_empty() { final_msg = format!("{}\n\n{}", prompts, final_msg); }
            }
            if let Err(e) = agent.prompt(&final_msg).await {
                let _ = discord_msg.edit(&http, EditMessage::new().content(format!("❌ Error: {}", e))).await;
                return;
            }
        }
        ...
        let typing_task = tokio::spawn(async move {
            loop { let _ = channel_id.broadcast_typing(&typing_http).await; tokio::time::sleep(std::time::Duration::from_secs(5)).await; }
        });
        ...
        while let Ok(event) = rx.recv().await {
            match event {
                AgentEvent::MessageUpdate { thinking: t, text: txt, is_delta } => {
                    if is_delta {
                        ...
                        if !t.is_empty() { composer.update_block_by_id("sync-thinking", BlockType::Thinking, t); }
                        if !txt.is_empty() { composer.update_block_by_id("sync-text", BlockType::Text, txt); }
                    }
                }
                ...
                AgentEvent::AutoRetry { attempt, max } => {
                    composer.push_delta(BlockType::Status, &format!("🔄 **自動重試 ({}/{})** - API 暫時限制中...", attempt, max));
                    last_upd = std::time::Instant::now() - std::time::Duration::from_secs(10);
                }
                AgentEvent::AgentEnd { success, error } => {
                    status = if success { ExecStatus::Success } else { ExecStatus::Error(error.unwrap_or_else(|| "Error".to_string())) };
                }
                AgentEvent::Error { message } => { 
                    composer.push_delta(BlockType::Status, &format!("⚠️ **錯誤:** {}", message)); 
                }
                _ => {}
            }
            ...
            if last_upd.elapsed() >= std::time::Duration::from_millis(1500) || status != ExecStatus::Running {
                ...
                match &status {
                    ExecStatus::Error(e) => { embed = embed.title(i18n.get("api_error")).color(0xff0000).description(format!("{}\n\n❌ **錯誤:** {}", desc, e)); }
                    ExecStatus::Success => { embed = embed.title(i18n.get("pi_response")).color(0x00ff00).description(if desc.is_empty() { i18n.get("done") } else { desc }); }
                    ExecStatus::Running => { embed = embed.title(i18n.get("pi_working")).color(0xFFA500).description(if desc.is_empty() { i18n.get("wait") } else { desc }); }
                }
                let _ = discord_msg.edit(&http, EditMessage::new().embed(embed)).await;
                last_upd = std::time::Instant::now();
                if status != ExecStatus::Running { typing_task.abort(); break; }
            }
        }
        typing_task.abort();
    }
...
impl EventHandler for Handler {
    ...
    async fn message(&self, ctx: Context, msg: Message) {
        ...
        info!("🔐 Auth status: is_auth={}, mention_only={}", is_auth, mention_only);

        if !is_auth {
            let is_dm = msg.guild_id.is_none();
            if is_dm || msg.mentions_me(&ctx).await.unwrap_or(false) {
                info!("🔒 Generating auth token for unauthorized user/channel");
                if let Ok(token) = self.state.auth.create_token(if is_dm { "user" } else { "channel" }, if is_dm { &user_id } else { &channel_id_str }) {
        ...
        match self.state.session_manager.get_or_create_session(msg.channel_id.get(), agent_type).await {
            Ok((agent, is_brand_new)) => {
                info!("✅ Session obtained (new={})", is_brand_new);
                let content = if msg.content.starts_with('!') { &msg.content[1..] } else { &msg.content }.to_string();
                let handler_state = self.state.clone();
                tokio::spawn(async move {
                    info!("🚀 Starting agent loop thread");
                    Handler::start_agent_loop(agent, ctx.http.clone(), msg.channel_id, handler_state, Some(content), is_brand_new).await;
                });
            },
            Err(e) => {
                error!("❌ Failed to get/create session: {}", e);
                let _ = msg.reply(&ctx.http, format!("❌ 無法初始化會話: {}", e)).await;
            }
        }
    }
        ...
        match interaction {
            ...
            Interaction::Component(component) => {
                ...
                if custom_id.starts_with("agent_") {
                    ...
                    if let Ok((agent, _)) = self.state.session_manager.get_or_create_session(component.channel_id.get(), agent_type).await {
...
impl Handler {
    async fn handle_slash_command(&self, ctx: &Context, command: &CommandInteraction) {
        ...
        if cmd_name == "agent" {
            for cmd in commands::get_all_commands() { if cmd.name() == cmd_name { let _ = cmd.execute(ctx, command, Arc::new(NoOpAgent)).await; return; } }
        }
        let channel_id_str = command.channel_id.to_string();
        let agent_type = ChannelConfig::load().await.unwrap_or_default().get_agent_type(&channel_id_str);
        if let Ok((agent, _)) = self.state.session_manager.get_or_create_session(command.channel_id.get(), agent_type).await {
            for cmd in commands::get_all_commands() { if cmd.name() == cmd_name { let _ = cmd.execute(ctx, command, agent).await; return; } }
        }
    }
}
...
async fn run_bot() -> anyhow::Result<()> {
    ...
    let state = AppState { config: config.clone(), session_manager: Arc::new(SessionManager::new(config.clone())), auth: Arc::new(AuthManager::new()), i18n: Arc::new(RwLock::new(I18n::new(&config.language))) };
    let token = state.config.discord_token.clone();
    let mut client = Client::builder(&token, GatewayIntents::GUILD_MESSAGES | GatewayIntents::MESSAGE_CONTENT | GatewayIntents::GUILDS | GatewayIntents::DIRECT_MESSAGES).event_handler(Handler { state }).await?;
    client.start().await?;
    Ok(())
}
...
fn manage_daemon(action: DaemonAction) -> anyhow::Result<()> {
    ...
    let service_file = base_dir.join("systemd").join(format!("{}.service", service_name));
    match action {
        DaemonAction::Enable => {
            ...
            let link = dirs::home_dir().unwrap().join(".config/systemd/user").join(format!("{}.service", service_name));
            let _ = std::fs::remove_file(&link);
            std::os::unix::fs::symlink(&service_file, &link)?;
            let _ = StdCommand::new("systemctl").arg("--user").arg("daemon-reload").status();
            let _ = StdCommand::new("systemctl").arg("--user").arg("enable").arg(service_name).status();
            let _ = StdCommand::new("systemctl").arg("--user").arg("start").arg(service_name).status();
        }
        DaemonAction::Disable => {
            ...
            let _ = std::fs::remove_file(dirs::home_dir().unwrap().join(".config/systemd/user").join(format!("{}.service", service_name)));
        }
    }
    Ok(())
}
...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Some(Commands::Reload) => {
            let _ = StdCommand::new("systemctl").arg("--user").arg("kill").arg("-s").arg("HUP").arg("agent-discord-rs").status();
            println!("✅ Reload sent.");
        }
        Some(Commands::Auth { token }) => {
            let manager = AuthManager::new();
            if let Ok((t, id)) = manager.redeem_token(&token) { println!("✅ Authorized {} {}.", t, id); }
        }
